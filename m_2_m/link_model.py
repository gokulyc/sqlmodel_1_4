...
from db import Hero, HeroTeamLink, Team, engine
...
def create_heroes():
    with Session(engine) as session:
        ...
        for link in team_z_force.hero_links:
            rprint("Z-Force hero:", link.hero, "is training:", link.is_training)

        for link in team_preventers.hero_links:
            rprint("Preventers hero:", link.hero, "is training:", link.is_training)
...
def update_heroes():
    with Session(engine) as session:
        ...
        spider_boy_z_force_link = HeroTeamLink(team=team_z_force, hero=hero_spider_boy, is_training=True)
